...
def extract_json(text):
    ...
    if match:
        ...
        try:
            ...
            print("Using fuzzy_json to parse JSON")
            from fuzzy_json import loads
            json_cells = loads(json_str)
            json_cells = []
...
def extract_enumerated_items(text):
    """
    Extracts arbitrarily enumerated list items.
    Supports formats like:
        - 1. Item text
        - (1) Item text
        - [1] Item text
        - {1} Item text
        - \n1. Item text (newline-separated lists)
    """
    # Regex pattern for detecting enumerations (handles numbers inside (), [], {}, or followed by .)
    pattern = r"(?:\n|^)[\(\[\{]?(\d+)[\)\]\}]?[.\s]+"

    # Find all occurrences of enumeration
    matches = list(re.finditer(pattern, text))

    items = []
    
    for i, match in enumerate(matches):
        start = match.end()  # Start of the item content
        end = matches[i + 1].start() if i + 1 < len(matches) else len(text)  # End of the item
        
        item_text = text[start:end].strip()  # Extract content and remove surrounding spaces
        items.append((int(match.group(1)), item_text))  # Store as tuple (number, content)

    # Sort by enumeration number (useful if unordered)
    items.sort()
...
def extract_subtasks(subtasks_str: str) -> List[str]:
    ...
    if not matches:
        matches = extract_enumerated_items(subtasks_str)
    if not matches:
...
def extract_code(llm_response: str) -> List[dict]:
    """
    format: [code][[<code cell content>]][/code]
    """
    pattern = r"\[code\](.*?)\[/code\]"
    ...
    return [{"cell_type": "code", "content": process_match(match)} for match in matches]
...
if __name__ == '__main__':
    ...
    data_paths = glob.glob(f"../../../read_only_sandbox/raw_data/Cb/**/*.h5ad", recursive=True)

    if len(data_paths) > 0:
        print(f"Found {len(data_paths)} .h5ad files")
        data = sc.read_h5ad(data_paths[0])
        # If you want to combine multiple files
        # for path in data_paths[1:]:
        #     data = data.concatenate(sc.read_h5ad(path))
    else:
        print("No h5ad files found in the specified directory")

    print("Data loading completed successfully")
    [[/code]]"""

    c2 = """```python
# Example usage after installing scanpy:
import h5ad

# Use h5ad to read an .h5ad file
adata = h5ad.read("your_file.h5ad")
```"""
    print(extract_code(c2))