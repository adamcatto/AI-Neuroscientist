...
def main_llm():
    ...
    tasks_str = query_llm(MAIN_TASK_PROMPT, temperature=0.25)
    print('tasks_str:', tasks_str)
    if '[[item]][[1]]' not in tasks_str:
        # Take extra care to properly format tasks string.
        _prompt = f"Given the generated subtasks represented by:\n {tasks_str}\nplease respond with a list of tasks in the format '[[item]][[1:N]][[description-1:N]]'.\
            It is very important that you respond with the exact format, otherwise the notebook generation will not work.\
            I will repeat: the format is '[[item]][[1:N]][[description-1:N]]'.\
            the format is '[[item]][[1:N]][[description-1:N]]'.\
            Alternatively, you can just separate the tasks with three newline characters: \\n\\n\\n and I will try to parse them."
        tasks_str = query_llm(_prompt, temperature=0.25)
    # add new markdown cell with the tasks
    add_cell_to_notebook(
        notebook_path,
        tasks_str,
        cell_type='markdown'
    ...
    for task_idx, task in enumerate(tasks):
        task = task + '\nhere is context about the files: ' + DATA_LOADING_FILE_CONTEXT_PROMPT + '\n \
            use that if you have not loaded in the data, otherwise ignore it. Also, if you need to concatenate anndata objects, \
            use import anndata as ad\nad.concat(adatas)'
        ...
        while not task_addressed and idx <= NUM_TRIES:
            ...
            for cell in cell_list:
                ...
                if cell_type == "code":
                    ...
                    while error_occured:
                        error_msg = cell_output # it's a dictionary, error message is the single value.
                        if not list(error_msg.values())[0]: # i.e. if the error message is empty
                            error_occured = False
                            break
                        print('debugging error_msg:', error_msg)
                        # debug the cell
                        cell_debugger = CodeCellDebugger(notebook_path)
                        cell_content, cell_output, error_occured = cell_debugger.chain_debug(cell_content, error_msg)
                        print('error occured:', error_occured)
                        time.sleep(1)
